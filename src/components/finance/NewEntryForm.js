...
const NumberInput = ({ value, onChange, disabled = false }) => {
  ...
  return (
    <input
      ...
      placeholder="0.00"
      disabled={disabled}
      className={`w-24 sm:w-32 px-2 sm:px-3 py-2 text-sm border border-gray-300 rounded-md text-right focus:ring-2 focus:ring-current text-gray-900 ${disabled ? "bg-gray-100 cursor-not-allowed" : "bg-white"
        }`}
    />
  );
};
    ...
    <div className="flex items-center justify-center mb-6 px-4">
      {[1, 2, 3].map((step) => (
        <React.Fragment key={step}>
          ...
          {step < 3 && (
            <div
              className={`w-16 sm:w-24 h-1 mx-2 transition-all ${currentStep > step ? "bg-blue-600" : "bg-gray-200"
                }`}
            />
          )}
        </React.Fragment>
